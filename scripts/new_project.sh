...
new_project(){
    ...
    find . -type f -name "*.xml" | while read i; do sed "s/$Foo/$ProjectName/g" -i "$i"; done
    find . -type f -name "*.xml" | while read i; do sed "s/$foo/$project_name/g" -i "$i"; done

    find . -type f -name "*.properties" | while read i; do sed "s/$Foo/$ProjectName/g" -i "$i"; done
    find . -type f -name "*.properties" | while read i; do sed "s/$foo/$project_name/g" -i "$i"; done
...
echo "" >> settings.gradle
echo "include '${project_name}_procedure'" >> settings.gradle
echo "include '${project_name}_efficiency'" >> settings.gradle
echo "include '${project_name}_solution'" >> settings.gradle
echo "project(':${project_name}_solution').projectDir = file('solutions/${project_name}_solution')" >> settings.gradle
