...
public class NaivesortProcedureWorld2d extends ProcedureWorld2d<NaivesortProcedureWorld2d, NaivesortProcedureDrawingOptions> {
	...
	public void buildAndSendManualModel() {
		...
		Carte[] source = new Carte[bottomCards2d.size()];
		...
		for(int i = 0; i < bottomCards2d.size(); i++) {
			...
			if(card2d.isNullCard()) {
				...
				source[i] = (Carte) card2d.getCard();
				
			}
		}

		Carte[] target = new Carte[topCards2d.size()];
	...
	private void addNullCardAtDraggedCardOriginalLocation(List<NaivesortCard2d> cards) {
		NullCard nullCard = new NullCard();
		NaivesortCard2d draggedCardLocation = new NaivesortCard2d(nullCard.id(), new NullCard());
